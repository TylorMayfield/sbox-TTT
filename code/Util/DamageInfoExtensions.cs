...
public static class DamageInfoExtensions
{
	...
	/// </summary>
	public static bool IsHeadshot( this DamageInfo info ) => info.Hitbox?.Tags.Has( "head" ) == true;

	/// <summary>
	/// If this damage ends up being lethal, should the player scream when killed?
	/// </summary>
	public static bool IsSilent( this DamageInfo info ) => info.Tags.Has( DamageTags.Silent );

	/// <summary>
	/// If this damage was avoidable e.g. a traitor dying to a
	/// teammate's C4, then no karma penalty will be given to the teammate.
	/// </summary>
	public static bool IsAvoidable( this DamageInfo info ) => info.Tags.Has( DamageTags.Avoidable );
}
